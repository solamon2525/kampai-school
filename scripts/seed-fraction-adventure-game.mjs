...
async function main() {
  console.log('Seeding Fraction Adventure game into database...');
  ...
  // 4. Seed item
  const url = '/games/math/fraction-adventure.html';
  ...
  if (!itemData || itemData.length === 0) {
    ...
    const { data: insertData, error: insertError } = await supabase
      ...
      .insert({
        ...
        item_type: 'link',
        title: '🍕 ผจญภัยเศษส่วน ป.4 — Fraction Adventure',
        external_url: url,
        subject: 'คณิตศาสตร์',
        sort_order: 254,
        game_slug: 'fraction-adventure',
        tracked_game: true,
        is_published: true,
  ...
  const { error: docError } = await supabase
    ...
    .upsert({
      item_id: itemId,
      owner_staff_id: staffId,
      game_format: 'เกมการเรียนรู้เรื่องเศษส่วน ป.4 (Like/Unlike Denominators, Word Problems, AR Mode)',
      features: features,
      version: 'v1.0.0',
      notes: 'ผจญภัยเศษส่วน ป.4 (programmatic seed)'
    }, { onConflict: 'item_id' });
