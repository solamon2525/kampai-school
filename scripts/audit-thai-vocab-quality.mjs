...
const OUT = join(ROOT, 'output/thai-vocab-quality-review.csv');
...
const rows = [['category_slug', 'category_title', 'status', 'word', 'reading', 'meaning', 'category_evidence', 'review_reason', 'duplicate_rationale']];
...
for (const category of categories) {
  ...
  for (const item of [...quarantined, ...approvedSample]) {
    rows.push([
      category.slug,
      category.title,
