...
async function main() {
  console.log('Seeding Attack on Noun game into database...');
  ...
  if (!itemData || itemData.length === 0) {
    ...
    const { data: insertData, error: insertError } = await supabase
      ...
      .insert({
        ...
        item_type: 'link',
        title: '🛡️ ผู้พิทักษ์ลักษณะนาม: ปะทะ 3 ขุนพลไททัน',
        external_url: url,
        subject: 'ภาษาไทย',
        sort_order: 20,
        game_slug: 'attack-on-noun',
        tracked_game: true,
        is_published: true,
        thumbnail_url: '/games/thai/attack-on-noun/cover.png',
        bgm_preset: 'cheerful'
      })
      .select('id');
  ...
  const features = [
    ...
    'โหมดควบคุมผ่านคีย์บอร์ด (WASD + Space + Mouse) เล็งยิงคำตอบลักษณะนาม',
  ...
  const { error: docError } = await supabase
    ...
    .upsert({
      item_id: itemId,
      owner_staff_id: staffId,
      game_format: '3D/FPS Action Shooter Game (ลักษณะนาม ภาษาไทย)',
      features: features,
      version: 'v1.0.0',
      notes: 'ผู้พิทักษ์ลักษณะนาม: ปะทะ 3 ขุนพลไททัน (programmatic seed)'
    }, { onConflict: 'item_id' });
