...
async function main() {
  ...
  const { error: docError } = await supabase
    ...
    .upsert({
      item_id: itemId,
      owner_staff_id: staffId,
      game_format: '2D Auto-Scrolling Platformer (Mario style) — ฉากไหลอัตโนมัติ 5 หัวใจ กระโดดข้ามหลุม/หลบมอนสเตอร์ โหม่งบล็อกคำตอบเปลี่ยนเป็นก้อนหินเหยียบได้',
      features: features,
      version: 'v1.2.0',
      notes: 'เพิ่มระบบหัวใจ 5 ดวง สุ่มโจทย์ใหม่อัตโนมัติเมื่อบล็อกพ้นจอ และเปลี่ยนบล็อกที่โหม่งแล้วเป็นก้อนหินเหยียบยืนได้',
      updated_at: new Date().toISOString()
    }, { onConflict: 'item_id' });
