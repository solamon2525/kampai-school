...
async function main() {
  console.log('Seeding Fraction Tank Battle game into database...');
  ...
  // 4. Seed item
  const url = '/games/math/fraction-tank/index.html';
  ...
  if (!itemData || itemData.length === 0) {
    ...
    const { data: insertData, error: insertError } = await supabase
      ...
      .insert({
        ...
        item_type: 'link',
        title: '🚀 Fraction Tank Battle — ศึกรถถังพิชิตน่านฟ้าเศษส่วน',
        external_url: url,
        subject: 'คณิตศาสตร์',
        sort_order: 366,
        game_slug: 'fraction-tank',
        tracked_game: true,
        is_published: true,
  ...
  const { error: docError } = await supabase
    ...
    .upsert({
      item_id: itemId,
      owner_staff_id: staffId,
      game_format: 'Horizontal/Arcade Canvas Tank Shooter + โจทย์เปรียบเทียบเศษส่วน (> หรือ <)',
      features: features,
      version: 'v1.0.0',
      notes: 'ศึกรถถังพิชิตน่านฟ้าเศษส่วน ป.3-6 (programmatic seed)'
    }, { onConflict: 'item_id' });
