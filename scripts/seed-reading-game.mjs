...
async function main() {
  console.log('Seeding Reading Adventure game into database...');
  ...
  // 4. Seed item
  const url = '/games/thai/reading-game/index.html';
  ...
  if (!itemData || itemData.length === 0) {
    ...
    const { data: insertData, error: insertError } = await supabase
      ...
      .insert({
        ...
        item_type: 'link',
        title: 'ผจญภัยอ่านสนุก (Reading Adventure)',
        external_url: url,
        subject: 'ภาษาไทย',
        sort_order: 150
      })
  ...
  const { error: docsError } = await supabase
    ...
    .upsert({
      item_id: itemId,
      owner_staff_id: staffId,
      game_format: 'ตอบคำถาม/เรียงประโยค/จับใจความ',
      features: ['จับคู่คำพ้องสระ', 'เรียงประโยค', 'เลือกการอ่านที่ถูกต้อง', 'ปริศนาหาคำ', 'อ่านจับใจความ'],
      version: 'v1.0.0',
