...
async function main() {
  console.log('Seeding Thai Spelling game into database...');
  ...
  // 4. Seed item
  const url = '/games/thai/thai-spelling.html';
  ...
  if (!itemData || itemData.length === 0) {
    ...
    const { data: insertData, error: insertError } = await supabase
      ...
      .insert({
        ...
        item_type: 'link',
        title: 'มาตราตัวสะกด (Thai Spelling)',
        external_url: url,
        subject: 'ภาษาไทย',
        sort_order: 160
      })
  ...
  const { error: docsError } = await supabase
    ...
    .upsert({
      item_id: itemId,
      owner_staff_id: staffId,
      game_format: 'ตอบคำถาม/พิมพ์สะกดคำ/จับเวลา',
      features: ['โหมดจับเวลา', 'โหมดเลือกมาตรา', 'โหมดยากขึ้นเรื่อยๆ', 'โหมดกลับกัน', 'ระบบเว้นระยะทบทวน (Spaced Repetition)'],
      version: 'v1.0.0',
