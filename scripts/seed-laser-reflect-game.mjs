...
async function main() {
  console.log('Seeding Laser Reflect game into database...');
  ...
  const url = '/games/math/laser-reflect/index.html';
  ...
  if (!itemData || itemData.length === 0) {
    ...
    const { data: insertData, error: insertError } = await supabase
      ...
      .insert({
        ...
        item_type: 'link',
        title: '📐 Laser Reflect — พิกัดวิถีสะท้อนเลเซอร์',
        external_url: url,
        subject: 'คณิตศาสตร์',
        sort_order: 394,
        game_slug: 'laser-reflect',
        tracked_game: true,
        is_published: true,
  ...
  const { error: docError } = await supabase
    ...
    .upsert({
      item_id: itemId,
      owner_staff_id: staffId,
      game_format: '2D Coordinate Grid / Laser Raycasting Reflection Puzzle',
      features: features,
      version: 'v1.0.0',
      notes: 'ลำแสงสะท้อนเรขาคณิต ป.4-6 (programmatic seed)'
    }, { onConflict: 'item_id' });
