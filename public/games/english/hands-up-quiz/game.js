/* game.js — เกม AR "ยกมือตอบ" (อ่าน config/data + KAMPAI SDK + KampaiAR engine, โหมด hands)
   ❗ ไม่มี camera code ที่นี่ — กล้อง/ตรวจจับ/ยกมืออยู่ใน kampai-ar.js ทั้งหมด
   โซน engine: 'left' (ยกมือซ้าย) · 'both' (ยกสองมือ) · 'right' (ยกมือขวา) */
(function () {
    'use strict';
    var CFG = window.GAME_CONFIG, DATA = window.GAME_DATA;
    var $ = function (id) { return document.getElementById(id); };
    // คอลัมน์ซ้าย→กลาง→ขวา = left / both / right · choices[0..2] ตามลำดับนี้
    var ZONE_IDX = { left: 0, both: 1, right: 2 };
    var ZONE_EL = { left: 'zoneLeft', both: 'zoneCenter', right: 'zoneRight' };
    var COLS = ['left', 'both', 'right'];
    ...
    function setStatus() {
        var icon = ar && ar.mode === 'camera' ? '🎥 ยกมือตอบ' : '✋ แตะ';
        $('status-tag').textContent = icon + (ST.sec > 0 ? ' · ⏱ ' + ST.sec + 's' : '');
    }

    // ── AR engine (โหมด hands) ──
    function buildAR() {
        return KampaiAR.create({
            video: '#arVideo', canvas: '#arCanvas',
            detector: CFG.DETECTOR, mode: CFG.MODE, zones: CFG.ZONES, holdMs: CFG.HOLD_MS, tuning: CFG.TUNING,
            onZone: function (zone) {
                COLS.forEach(function (z) { $(ZONE_EL[z]).classList.toggle('active-zone', z === zone); });
            },
            onHoldProgress: function (zone, pct) {
                if (!zone || !ZONE_EL[zone]) return;
    ...
    function commitAnswer(zone) {
        ...
        var correctZone = COLS[q.answer];                // เฉลย: ไฮไลต์คอลัมน์ที่ถูก
        $(ZONE_EL[correctZone]).classList.add('correct');
        if (zone && ZONE_EL[zone] && !correct) $(ZONE_EL[zone]).classList.add('wrong');
        if (correct) {
