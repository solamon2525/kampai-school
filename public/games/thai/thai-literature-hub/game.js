/* game.js — คลังอ่านจับใจความ ป.4-5 */
...
const FLASH_VARIANT_KEY = 'tlh_flash_variant';
...
function flashKnownKey(slug) {
  return `tlh_flash_known_${slug || 'default'}`;
}
...
const GRID_GRADE_KEY = 'tlh_grid_grade';
const GRID_VIEW_KEY = 'tlh_grid_view';
...
function switchMode(mode) {
  ...
  const scoredModes = ['dictation', 'choice', 'match', 'listen', 'class', 'reading'];
...
function loadClassQuizWord() {
  ...
  document.getElementById('wfront-cat').textContent = 'อ่าน+ข้อคิด';
...
const AUTO_READ_KEY = 'tlh_auto_read_mode';
const AUTO_PAUSE_MS_KEY = 'tlh_auto_pause_ms';
const AUTO_FLIP_KEY = 'tlh_auto_flip';
const AUTO_SOUND_KEY = 'tlh_auto_sound';
...
if (typeof KampaiMatch !== 'undefined' && KampaiMatch.create) {
  KampaiMatch.create({ slug: (CONFIG && CONFIG.SLUG) || 'thai-literature-hub' });
}
