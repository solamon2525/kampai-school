/* game.js — คลังอักษรไทย ป.1-4: ไตรยางศ์ พยัญชนะ 3 หมู่ สระ วรรณยุกต์ */
...
const FLASH_VARIANT_KEY = 'tsh_flash_variant';
...
function applyMissedFromSdk() {
  /* script hub ใช้ local missed ในรอบฝึกเท่านั้น */
}
...
function flashKnownKey(slug) {
  return `tsh_flash_known_${slug || 'default'}`;
}
...
const GRID_GRADE_KEY = 'tsh_grid_grade';
const GRID_VIEW_KEY = 'tsh_grid_view';
...
function getSpeakText(item) {
  if (!item) return '';
  if (item.char_class || item.item_type === 'consonant') {
    return item.reading || item.name || item.word;
  }
  if (item.item_type === 'vowel' || item.item_type === 'vowel-compound' || item.item_type === 'tone') {
    return item.reading || item.word;
  }
...
function getClassBadgeHtml(item) {
  if (!item.char_class) return '';
  const short = { high: 'สูง', mid: 'กลาง', low: 'ต่ำ' }[item.char_class] || '';
  return short ? `<span class="grid-class-badge grid-class-badge--${item.char_class}">${short}</span>` : '';
}
...
function pickWordDecoys(wordItem, pool, count) {
  if (wordItem.char_class) {
    const same = pool.filter((w) => w.char_class === wordItem.char_class && w.word !== wordItem.word);
    if (same.length >= count) return shuffle([...same]).slice(0, count);
    const other = pool.filter((w) => w.char_class && w.char_class !== wordItem.char_class);
    if (other.length) return shuffle([...same, ...other]).slice(0, count);
  }
...
function initHubGrid() {
  ...
  CATEGORIES.forEach(cat => {
    ...
    const progBlock = pct > 0
      ...
      : (count ? `<span class="hc-prog-pct">${count} รายการ</span>` : '');
...
function switchMode(mode) {
  ...
  const scoredModes = ['dictation', 'choice', 'match', 'listen', 'class'];
...
function renderWordsGrid() {
  ...
  if (activeCategory) {
    ...
    const countLabel = gridDisplayWords.length === fullCategoryWords.length
      ? `${fullCategoryWords.length} รายการ`
      : `แสดง ${gridDisplayWords.length} / ${fullCategoryWords.length} รายการ`;
    header.innerHTML = `
  ...
  gridDisplayWords.forEach((item, idx) => {
    ...
    const classBadge = getClassBadgeHtml(item);
...
function loadWord(index) {
  ...
  document.getElementById('btn-say').onclick = (e) => {
    e.stopPropagation();
    speakThai(getSpeakText(wordItem));
  };
...
function playVoiceOnly() {
  if (currentWordIndex < quizList.length) {
    speakThai(getSpeakText(quizList[currentWordIndex]));
  }
}
...
function loadClassQuizWord() {
  ...
  if (wordItem.char_class) {
    questionText = `อักษร "${wordItem.word}" อยู่หมู่ใด?`;
    currentQuizAnswer = CLASS_LABELS[wordItem.char_class];
    const decoys = Object.values(CLASS_LABELS).filter((l) => l !== currentQuizAnswer);
    options = shuffle([currentQuizAnswer, ...shuffle(decoys).slice(0, 3)]);
  } else if (wordItem.item_type === 'tone-rule') {
    questionText = `รูปแบบ "${wordItem.word}" ได้ผลลัพธ์อย่างไร?`;
    currentQuizAnswer = wordItem.tone_result || wordItem.meaning;
    const decoys = shuffle(pool.filter((w) => w.item_type === 'tone-rule' && w !== wordItem)
      .map((w) => w.tone_result || w.meaning)).slice(0, 3);
    options = shuffle([currentQuizAnswer, ...decoys]);
  } else if (wordItem.item_type === 'leading') {
    questionText = `คำ "${wordItem.word}" — กฎอักษรนำคือข้อใด?`;
    currentQuizAnswer = wordItem.rule || (wordItem.meaning || '').split(' · ')[0];
    const decoys = shuffle(pool.filter((w) => w.item_type === 'leading' && w !== wordItem)
      .map((w) => w.rule || (w.meaning || '').split(' · ')[0])).slice(0, 3);
    options = shuffle([currentQuizAnswer, ...decoys]);
  } else if (wordItem.item_type === 'mattra') {
    questionText = `มาตรา "${wordItem.word}" — ข้อใดถูกต้อง?`;
    currentQuizAnswer = wordItem.meaning;
    const decoys = shuffle(pool.filter((w) => w.item_type === 'mattra' && w !== wordItem)
      .map((w) => w.meaning)).slice(0, 3);
    options = shuffle([currentQuizAnswer, ...decoys]);
  } else if (wordItem.item_type === 'triyang' || wordItem.item_type === 'triyang-advanced') {
    questionText = `คำ "${wordItem.word}" — ส่วนประกอบ / แยกพยางค์คือข้อใด?`;
    currentQuizAnswer = wordItem.reading || wordItem.syllables || wordItem.parts;
    const decoys = shuffle(pool.filter((w) => w !== wordItem && (w.reading || w.syllables))
      .map((w) => w.reading || w.syllables)).slice(0, 3);
    options = shuffle([currentQuizAnswer, ...decoys]);
  } else {
    questionText = `"${wordItem.word}" — ข้อใดถูกต้อง?`;
    currentQuizAnswer = wordItem.meaning;
    options = shuffle([currentQuizAnswer, ...pickMeaningDecoys(wordItem, pool, 3)]);
  }

  document.getElementById('choice-question-text').textContent = questionText;
  document.getElementById('wfront-emoji').textContent = wordItem.emoji || '🎯';
  document.getElementById('wfront-word').textContent = wordItem.word;
  document.getElementById('wfront-cat').textContent = 'ทายหมู่ / กฎ';
  document.getElementById('wback-reading').textContent = wordItem.reading || '—';
  document.getElementById('wback-meaning').textContent = 'เฉลยหลังตอบ';
  document.getElementById('tcard').classList.remove('flipped');
  ...
  options.forEach((opt) => {
    ...
    btn.textContent = opt;
    btn.onclick = () => selectClassAnswer(opt, btn);
...
function selectClassAnswer(selectedOpt, clickedBtn) {
  if (isAnswered) return;
  isAnswered = true;

  const wordItem = quizList[currentWordIndex];
  const correct = currentQuizAnswer || wordItem.meaning;
  ...
  if (selectedOpt === correct) {
    ...
    btns.forEach((btn) => { if (btn.textContent === correct) btn.classList.add('correct'); });
    currentLives--;
    recordAnswer(wordItem, false);
    showToast('ยังไม่ถูกนะ 😢', 'wrong');
    KAMPAI.sound.wrong();
  }

  document.getElementById('wback-meaning').textContent = correct;
  document.getElementById('tcard').classList.add('flipped');
  document.getElementById('bar').style.width = `${((currentWordIndex + 1) / quizList.length) * 100}%`;
...
// ═══ MODE 4: CHOICE QUIZ (ทายความหมาย) ═══
...
function loadChoiceWord() {
  ...
  let questionText = `รายการ "${wordItem.word}" หมายความว่าข้อใด?`;
  document.getElementById('choice-question-text').textContent = questionText;
...
function replayListenWord() {
  if (currentWordIndex < quizList.length) speakThai(getSpeakText(quizList[currentWordIndex]));
}
...
function revealFlashCard() {
  ...
  if (item && flashVariant !== 'audio2word') speakThai(getSpeakText(item));
}
...
const AUTO_READ_KEY = 'tsh_auto_read_mode';
const AUTO_PAUSE_MS_KEY = 'tsh_auto_pause_ms';
const AUTO_FLIP_KEY = 'tsh_auto_flip';
const AUTO_SOUND_KEY = 'tsh_auto_sound';
...
function goToAutoplayIndex(idx, opts) {
  ...
  if (options.speak && gridDisplayWords[currentWordIndex]) {
    speakThai(getSpeakText(gridDisplayWords[currentWordIndex]));
  }
}
...
document.addEventListener('keydown', (e) => {
  ...
  if (currentMode === 'auto') {
    ...
    const btn = btns[parseInt(e.key, 10) - 1];
    if (btn && !btn.disabled) btn.click();
  } else if (currentMode === 'class' && e.key >= '1' && e.key <= '4') {
    const btns = document.querySelectorAll('#choice-options .option-btn');
    const btn = btns[parseInt(e.key, 10) - 1];
    if (btn && !btn.disabled) btn.click();
  }
...
if (typeof KampaiMatch !== 'undefined' && KampaiMatch.create) {
  KampaiMatch.create({ slug: (CONFIG && CONFIG.SLUG) || 'thai-script-hub' });
}
