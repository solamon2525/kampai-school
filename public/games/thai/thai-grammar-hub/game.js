/* game.js — คลังไวยากรณ์ไทย ป.4-5: ชนิดและหน้าที่ของคำ */
...
const FLASH_VARIANT_KEY = 'tgh_flash_variant';
...
function flashKnownKey(slug) {
  return `tgh_flash_known_${slug || 'default'}`;
}
...
const GRID_GRADE_KEY = 'tgh_grid_grade';
const GRID_VIEW_KEY = 'tgh_grid_view';
const RANDOM_SAMPLE = 20;
let currentQuizAnswer = '';

function getGridBackReading(item) {
  if (!item) return '';
  return item.pos_label || item.reading || '';
}

function getGridBackMeaning(item) {
  if (!item) return '';
  const bits = [];
  if (item.sentence) bits.push(item.sentence);
  const gloss = (item.meaning || '').split(' · ประโยค:')[0].trim();
...
function setTcardBack(item) {
  ...
  readingEl.textContent = r ? `ชนิด: [ ${r} ]` : '—';
  meaningEl.textContent = m || '—';
}

function getSpeakText(item) {
  if (!item) return '';
  if (item.sentence) return `${item.word} — ${item.sentence}`;
  return item.word || '';
}

function getPosBadgeHtml(item) {
  if (!item?.pos_label || !item.pos) return '';
  return `<span class="grid-pos-badge grid-pos-badge--${item.pos}">${item.pos_label}</span>`;
}
...
function renderWordsGrid() {
  ...
  gridDisplayWords.forEach((item, idx) => {
    ...
    const classBadge = getPosBadgeHtml(item);
...
function loadClassQuizWord() {
  ...
  document.getElementById('wfront-word').textContent = target;
  document.getElementById('wfront-cat').textContent = 'ทายชนิดคำ';
  document.getElementById('wback-reading').textContent = '— ซ่อนเฉลย —';
...
function initMatchingGame() {
  ...
  if (activeCategorySlug === 'antonyms') {
    ...
    rightItems = shuffle(matchWords.map((w) => ({ id: w.word, text: w.pos_label || w.reading })));
  }
...
const AUTO_READ_KEY = 'tgh_auto_read_mode';
const AUTO_PAUSE_MS_KEY = 'tgh_auto_pause_ms';
const AUTO_FLIP_KEY = 'tgh_auto_flip';
const AUTO_SOUND_KEY = 'tgh_auto_sound';
...
if (typeof KampaiMatch !== 'undefined' && KampaiMatch.create) {
  KampaiMatch.create({ slug: (CONFIG && CONFIG.SLUG) || 'thai-grammar-hub' });
}
