        ...
        <main>
          ...
          <section className="bg-gradient-to-br from-primary via-primary/95 to-primary/90 py-5 md:py-10 shadow-inner">
            <div className="container mx-auto px-4 text-center">
              ...
              <h1 className="text-xl md:text-2xl lg:text-3xl font-extrabold text-primary-foreground mb-1.5">
                {settings.school_name}
              </h1>
              <p className="text-[11px] md:text-sm text-primary-foreground/80 max-w-2xl mx-auto leading-relaxed">
                {settings.school_description}
              </p>
            </div>
          </section>

          {/* Vision Mission Section — Compact 2x2 Grid on Mobile */}
                ...
                {cardConfigs.map((c) => {
                  ...
                  return (
                    <div
                      ...
                      className={`group ${!bg ? 'bg-card' : ''} rounded-2xl p-3.5 md:p-6 shadow-sm hover:shadow-xl transition-all duration-300 hover:-translate-y-0.5 border border-border flex flex-col`}
                    >
                      {img ? (
                        <img src={img} alt={c.title} className="w-10 h-10 md:w-12 md:h-12 rounded-xl object-cover mb-2.5" />
                      ) : (
                        <div className="w-9 h-9 md:w-12 md:h-12 rounded-xl bg-primary/10 flex items-center justify-center mb-2.5 group-hover:bg-primary group-hover:scale-105 transition-all duration-300">
                          <c.icon className="w-5 h-5 md:w-6 md:h-6 text-primary group-hover:text-primary-foreground transition-colors" />
                        </div>
                      )}
                      <h3 className={`text-sm md:text-lg font-bold text-foreground mb-1 md:mb-2 ${alignClass}`}>{c.title}</h3>
                      <div
                        className={`text-foreground/80 text-xs md:text-sm leading-relaxed ${alignClass} prose prose-sm max-w-none flex-1`}
          ...
          {/* Philosophy · Motto · Identity · Colors — Compact 2x2 Grid on Mobile */}
          <section className="py-4 md:py-8 bg-secondary/10 border-y border-border">
            <div className="container mx-auto px-4">
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
                {/* ปรัชญา */}
                <div className="text-center p-3.5 md:p-5 bg-card rounded-2xl shadow-sm border border-border hover:shadow-md transition-shadow">
                  <div className="w-8 h-8 md:w-10 md:h-10 rounded-full bg-primary/10 flex items-center justify-center mx-auto mb-1.5">
                    <span className="text-lg">📖</span>
                  </div>
                  <div className="text-[9px] md:text-xs font-bold text-primary uppercase tracking-widest mb-0.5 md:mb-1">ปรัชญาโรงเรียน</div>
                  <p className="text-xs md:text-base font-extrabold text-foreground mb-0.5 leading-snug">
                    {settings.school_philosophy || 'นัตถิ ปัญญา สมา อาภา'}
                  </p>
                  <p className="text-[10px] md:text-xs text-foreground/70 italic leading-snug">
                    {settings.school_philosophy_translation || 'แสงสว่างเสมอด้วยปัญญาไม่มี'}
                  </p>
                </div>
                {/* คำขวัญ */}
                <div className="text-center p-3.5 md:p-5 bg-card rounded-2xl shadow-sm border border-border hover:shadow-md transition-shadow">
                  <div className="w-8 h-8 md:w-10 md:h-10 rounded-full bg-accent/10 flex items-center justify-center mx-auto mb-1.5">
                    <span className="text-lg">🎯</span>
                  </div>
                  <div className="text-[9px] md:text-xs font-bold text-primary uppercase tracking-widest mb-0.5 md:mb-1">คำขวัญโรงเรียน</div>
                  <p className="text-sm md:text-xl font-bold text-foreground leading-snug">
                ...
                <div className="text-center p-3.5 md:p-5 bg-card rounded-2xl shadow-sm border border-border hover:shadow-md transition-shadow">
                  ...
                  <div className="text-[9px] md:text-xs font-bold text-primary uppercase tracking-widest mb-0.5 md:mb-1">อัตลักษณ์โรงเรียน</div>
                  <p className="text-sm md:text-xl font-bold text-foreground leading-snug">
                ...
                <div className="text-center p-3.5 md:p-5 bg-card rounded-2xl shadow-sm border border-border hover:shadow-md transition-shadow">
                  <div className="flex justify-center gap-1.5 mb-1.5">
                    <div className="w-6 h-6 md:w-8 md:h-8 rounded-full bg-white border border-border shadow-sm" title="สีขาว" />
                    <div className="w-6 h-6 md:w-8 md:h-8 rounded-full bg-primary shadow-sm" title="สีเขียว" />
                  </div>
                  <div className="text-[9px] md:text-xs font-bold text-primary uppercase tracking-widest mb-0.5 md:mb-1">สีประจำโรงเรียน</div>
                  <p className="text-xs md:text-lg font-bold text-foreground">สีขาว และสีเขียว</p>
                </div>
              </div>
            </div>
          </section>

          {/* Stats Section — Compact 2x2 Grid on Mobile to prevent text squeeze */}
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
                {[
                  ...
                  <div key={index} className="text-center">
                    <stat.icon className="w-6 h-6 md:w-9 md:h-9 text-accent mx-auto mb-1 md:mb-2" />
                    <div className="text-lg md:text-3xl lg:text-4xl font-extrabold text-accent mb-0.5 leading-none">{stat.value}</div>
                    <div className="text-[9px] md:text-sm text-primary-foreground/90 leading-tight">{stat.label}</div>
                  </div>
                ))}
              </div>
            </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-2 md:gap-3 max-w-2xl mx-auto">
                  {[
                    ...
                    <div key={letter} className="text-center p-2.5 md:p-3 bg-card rounded-xl border border-border">
                      ...
                      <div className="text-[9px] md:text-xs text-foreground/70">{desc}</div>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div className="container mx-auto px-4">
              ...
              <div className="max-w-3xl mx-auto">
                {milestones.map((milestone, index) => (
                  <div key={index} className="flex gap-2.5 md:gap-5 mb-2.5 md:mb-4 last:mb-0">
                    <div className="flex-shrink-0 w-12 md:w-20 text-right">
                      <span className="text-xs md:text-xl font-bold text-primary leading-none">{milestone.year}</span>
                    </div>
                    <div className="relative">
                      <div className="w-2.5 h-2.5 md:w-4 md:h-4 rounded-full bg-accent border-2 border-background" />
                      {index < milestones.length - 1 && (
                        <div className="absolute top-2.5 md:top-4 left-1 w-0.5 h-full bg-border" />
                      )}
                    </div>
                    <div className="flex-1 pb-2.5">
                      <div className="bg-card rounded-xl p-3 md:p-4 shadow-sm border border-border">
                        <p className="text-foreground text-xs md:text-base leading-relaxed">{milestone.event}</p>
                      </div>
                    </div>
                  </div>
                ))}
            ...
            <div className="container mx-auto px-4">
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-5">
                {facilities.map((facility) => {
                  ...
                  return (
                    <div key={facility.id} className="bg-card rounded-2xl p-3.5 md:p-6 shadow-sm border border-border text-center flex flex-col justify-center items-center">
                      <div className="w-9 h-9 md:w-14 md:h-14 rounded-full bg-primary/10 flex items-center justify-center mb-2">
                        <IconComponent className="w-5 h-5 md:w-7 md:h-7 text-primary" />
                      </div>
                      <h3 className="text-xs md:text-lg font-bold text-foreground mb-1 leading-tight">{facility.title}</h3>
                      <p className="text-foreground/75 text-[10px] md:text-sm leading-relaxed">{facility.description}</p>
                    </div>
                  );
                })}
              </div>
