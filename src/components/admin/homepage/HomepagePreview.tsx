...
// VirtueBank preview
const VirtueBankPreview = () => {
    const [rows, setRows] = useState<Array<{ name: string; xp: number }> | null>(null);
    useEffect(() => {
        conductService.getTop10Heroes(5)
            .then(({ data }) => {
                setRows(
                    (data ?? []).map((r) => ({
                        name: r.name ?? '-',
    ...
    return (
        <div className="bg-white border border-gray-200 rounded-lg overflow-hidden">
            <div className="bg-slate-900 text-yellow-400 px-2 py-1 text-[7px] font-semibold flex items-center gap-1">
                <span>🌟</span> ธนาคารความดี — Top 5
            </div>
            {rows === null ? (
                <div className="px-2 py-2 text-[6px] text-gray-400 text-center">กำลังโหลด…</div>
            ) : rows.length === 0 ? (
                ...
                <ul className="divide-y divide-gray-100">
                    {rows.map((r, i) => (
                        <li key={i} className="flex items-center gap-1 px-2 py-1">
                            <span className="text-[7px] w-3 text-center">{WASTE_MEDALS[i]}</span>
                            <span className="text-[6px] text-gray-700 flex-1 truncate">{r.name}</span>
                            <span className="text-[6px] font-bold text-amber-700">+{r.xp} pt</span>
                        </li>
                    ))}
                </ul>
            )}
