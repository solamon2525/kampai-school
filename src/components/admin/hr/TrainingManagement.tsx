...
import { Plus, Pencil, Trash2, Search, FileText, Award } from 'lucide-react';
...
export default function TrainingManagement() {
  ...
  function openCreate() {
    setEditing(null);
    setForm({ staff_id: '', course_name: '', provider: '', training_type: 'อบรม', start_date: new Date().toISOString().split('T')[0], end_date: '', hours: 0, location: '', budget: 0, certificate_url: '', status: 'สมัครแล้ว', notes: '' });
    setDialogOpen(true);
  }
        ...
        <Table>
          ...
          <TableBody>
            {loading ? (
              ...
              <TableRow key={r.id}>
                <TableCell>{(r.staff as any)?.name || '-'}</TableCell>
