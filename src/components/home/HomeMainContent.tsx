...
const HomeMainContent = () => {
  ...
  return (
    <div className="flex flex-col gap-4">
      {/* Image Slider */}
      <div className="relative bg-gray-900 rounded-lg overflow-hidden aspect-[16/7] shadow-md">
        {slides.map((slide, i) => (
          <div
            key={i}
                ...
                <div className="relative h-44 sm:h-52 bg-gradient-to-br from-purple-600 to-indigo-700 overflow-hidden">
                  {featured.cover_image_url ? (
                    ...
                    <div className="w-full h-full flex items-center justify-center">
                      <span className="text-white/40 text-6xl font-bold">ข่าว</span>
                    </div>
                  )}
                  <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent" />
                  <div className="absolute top-2 left-2">
                    <span className={`text-white text-xs px-2 py-0.5 rounded-full font-medium ${categoryColor[featured.category] || 'bg-gray-600'}`}>
                      {featured.category}
                    </span>
                  </div>
                ...
                <Link key={item.id} to={`/news`} className="group p-0 block">
                  <div className="relative h-28 overflow-hidden bg-gray-100">
                    ...
                    <div className="absolute top-1.5 left-1.5">
                      <span className={`text-white text-[10px] px-1.5 py-0.5 rounded-full font-medium ${categoryColor[item.category] || 'bg-gray-600'}`}>
                        {item.category}
                      </span>
                    </div>
