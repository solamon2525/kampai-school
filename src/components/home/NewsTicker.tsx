...
const NewsTicker = () => {
  ...
  const renderItem = (item: typeof items[number], idx: number) => {
    ...
    return isExternal(item.link) ? (
      <a key={key} href={item.link} target="_blank" rel="noopener noreferrer" className={linkClass}>
        {content}
      </a>
    ) : (
      <Link key={key} to={item.link} className={linkClass}>
        {content}
      </Link>
    );
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div
          className="news-ticker-track absolute top-0 bottom-0 left-0 inline-flex items-center whitespace-nowrap text-sm font-medium"
          style={{
