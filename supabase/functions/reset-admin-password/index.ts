...
serve(async (req) => {
  ...
  try {
    ...
    const { administratorId, newPassword } = await req.json();
    if (!administratorId || !newPassword) {
      return json({ error: 'administratorId และ newPassword จำเป็นต้องกรอก' }, 400);
    }
    ...
    if (linkError || !link) {
      return json({ error: 'ไม่พบ account ของผู้บริหารคนนี้' }, 404);
    }
