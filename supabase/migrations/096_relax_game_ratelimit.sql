-- Reduce record_game_session rate limit: 20s → 5s
-- Math Jumper and similar fast games can legitimately end in under 20 seconds,
-- causing false rate_limited errors on back-to-back plays.
...
BEGIN
  ...
  -- 3) rate-limit: 1 session per 5s per (student, game)  ← was 20s
  IF EXISTS (
