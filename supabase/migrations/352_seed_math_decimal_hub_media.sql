-- 352: Seed Math Decimal Hub (media)

DO $$
DECLARE v_staff_id UUID; v_cat_media UUID; v_item_id UUID;
  v_url TEXT := '/games/math/math-decimal-hub/index.html';
BEGIN
  ...
  SELECT v_staff_id, v_cat_media, 'link', '🔢 คลังทศนิยม — อ่าน · เทียบ · บวกลบ · เงิน',
    'สื่อการสอนคณิตศาสตร์ ป.4-5: อ่านทศนิยม เปรียบเทียบ บวกลบ เงินทอน · อัปเกรดจาก decimal-media',
    v_url, '/games/math/math-decimal-hub/cover.png', 'คณิตศาสตร์', ARRAY['ป.4','ป.5'], ARRAY['ทศนิยม','เงิน','คณิตศาสตร์'], 96, false, true
  WHERE NOT EXISTS (SELECT 1 FROM public.educational_hub_items WHERE owner_staff_id = v_staff_id AND external_url = v_url);
  UPDATE public.educational_hub_items SET title = '🔢 คลังทศนิยม — อ่าน · เทียบ · บวกลบ · เงิน',
    thumbnail_url = '/games/math/math-decimal-hub/cover.png', sort_order = 96,
  ...
  VALUES (v_item_id, v_staff_id, 'คลังทศนิยม ป.4-5', ARRAY['อ่านหลัก','เปรียบเทียบ','บวกลบ','เงินทอน'], 'v1.0.0', 'TEACHING-MEDIA-IDEAS #11')
