-- ============================================================================
-- Migration 167: Seed "บอลลูนไฟเตอร์" (balloon-fighter)
-- ============================================================================
-- Owner: ครูณัฐพงศ์ สิงห์ชมภู
-- Path: public/games/thai/balloon-fighter/index.html
-- Cover: public/games/thai/balloon-fighter/cover.svg
...
BEGIN
  ...
  SELECT v_staff_id, v_cat_games, 'link', 'บอลลูนไฟเตอร์ (Balloon Fighter)', v_url, 'ภาษาไทย', 140
  WHERE NOT EXISTS (
  ...
  SET game_slug = 'balloon-fighter',
      tracked_game = true,
      is_published = true,
      thumbnail_url = '/games/thai/balloon-fighter/cover.svg',
      updated_at = now()
  WHERE owner_staff_id = v_staff_id AND external_url = v_url;
END $$;
