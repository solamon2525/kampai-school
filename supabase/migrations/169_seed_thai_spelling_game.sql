-- ============================================================================
-- Migration 169: Seed "มาตราตัวสะกด" (thai-spelling)
-- ============================================================================
-- Owner: ครูณัฐพงศ์ สิงห์ชมภู
-- Path: public/games/thai/thai-spelling.html
-- Cover: public/games/thai/thai-spelling-cover.png
...
DECLARE
  v_staff_id  UUID;
  v_cat_games UUID;
  v_url       TEXT := '/games/thai/thai-spelling.html';
BEGIN
  -- 1. Resolve staff_id
  ...
  SELECT v_staff_id, v_cat_games, 'link', 'มาตราตัวสะกด (Thai Spelling)', v_url, 'ภาษาไทย', 160
  WHERE NOT EXISTS (
  ...
  SET game_slug = 'thai-spelling',
      tracked_game = true,
      is_published = true,
      thumbnail_url = '/games/thai/thai-spelling-cover.png',
      updated_at = now()
  WHERE owner_staff_id = v_staff_id AND external_url = v_url;
  ...
  SELECT i.id, i.owner_staff_id,
         'ตอบคำถาม/พิมพ์สะกดคำ/จับเวลา',
         ARRAY['โหมดจับเวลา','โหมดเลือกมาตรา','โหมดยากขึ้นเรื่อยๆ','โหมดกลับกัน','ระบบเว้นระยะทบทวน (Spaced Repetition)'],
         'v1.0.0',
         'สร้างเกมครั้งแรก'
