-- Migration 435: lesson_packs + lesson_pack_items (Phase 15)
-- Bundles media + worksheet (+ optional game) into a teachable pack.
-- Seed via scripts/apply-migration-435-lesson-packs.mjs
-- Note: if a prior incomplete lesson_packs existed, use 435b_lesson_packs_recreate.sql
...
CREATE INDEX IF NOT EXISTS idx_lesson_packs_published
  ON public.lesson_packs (is_published, sort_order);
CREATE INDEX IF NOT EXISTS idx_lesson_packs_grades
...
CREATE INDEX IF NOT EXISTS idx_lesson_pack_items_pack
...
COMMENT ON TABLE public.lesson_packs IS
  'Phase 15 lesson packs — bundle media+worksheet(+game) for one teachable topic';
COMMENT ON TABLE public.lesson_pack_items IS
  'Items inside a lesson pack (role: media|worksheet|game)';
