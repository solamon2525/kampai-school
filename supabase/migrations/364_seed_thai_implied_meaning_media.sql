-- 364: Seed ความหมายโดยนัย media

DO $$
DECLARE v_staff_id UUID; v_cat_media UUID; v_item_id UUID;
  v_url TEXT := '/games/thai/thai-implied-meaning-media.html';
BEGIN
  ...
  SELECT v_staff_id, v_cat_media, 'link', '🔍 ความหมายโดยนัย',
    'สื่อการสอนภาษาไทย ป.5: แยกความหมายตรงตัวกับความหมายโดยนัย · ฝึกทายความหมายซ่อน',
    v_url, '/games/thai/thai-implied-meaning-media-cover.png', 'ภาษาไทย', ARRAY['ป.5'], ARRAY['โดยนัย','การอ่าน','สำนวน'], 103, false, true
  WHERE NOT EXISTS (SELECT 1 FROM public.educational_hub_items WHERE owner_staff_id = v_staff_id AND external_url = v_url);
  UPDATE public.educational_hub_items SET title = '🔍 ความหมายโดยนัย',
    thumbnail_url = '/games/thai/thai-implied-meaning-media-cover.png', sort_order = 103,
  ...
  VALUES (v_item_id, v_staff_id, 'โดยนัย ป.5', ARRAY['เรียนรู้','ฝึกทาย','12 ข้อ'], 'v1.0.0', 'ช่องว่างหลักสูตร ท 1.1 ป.5')
