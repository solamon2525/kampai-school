-- 353: Seed Math Geometry Hub (media)

DO $$
DECLARE v_staff_id UUID; v_cat_media UUID; v_item_id UUID;
  v_url TEXT := '/games/math/math-geometry-hub/index.html';
BEGIN
  ...
  SELECT v_staff_id, v_cat_media, 'link', '📐 คลังเรขาคณิต — มุม · เส้นรอบ · พื้นที่',
    'สื่อการสอนคณิตศาสตร์ ป.4-5: มุม แหลมฉากป้าน เส้นรอบรูป พื้นที่สี่เหลี่ยม รูป 2D · ต่อจาก angle-media',
    v_url, '/games/math/math-geometry-hub/cover.png', 'คณิตศาสตร์', ARRAY['ป.4','ป.5'], ARRAY['เรขาคณิต','มุม','พื้นที่','เส้นรอบ'], 97, false, true
  WHERE NOT EXISTS (SELECT 1 FROM public.educational_hub_items WHERE owner_staff_id = v_staff_id AND external_url = v_url);
  UPDATE public.educational_hub_items SET title = '📐 คลังเรขาคณิต — มุม · เส้นรอบ · พื้นที่',
    thumbnail_url = '/games/math/math-geometry-hub/cover.png', sort_order = 97,
  ...
  VALUES (v_item_id, v_staff_id, 'คลังเรขาคณิต ป.4-5', ARRAY['มุม','เส้นรอบ','พื้นที่','รูป2D'], 'v1.0.0', 'TEACHING-MEDIA-IDEAS #13')
