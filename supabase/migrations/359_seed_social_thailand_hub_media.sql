-- 359: Seed Social Thailand Hub (media)

DO $$
DECLARE v_staff_id UUID; v_cat_media UUID; v_item_id UUID;
  v_url TEXT := '/games/social/social-thailand-hub/index.html';
BEGIN
  ...
  SELECT v_staff_id, v_cat_media, 'link', '🇹🇭 คลังสังคมศึกษาไทย — แผนที่ · สุโขทัย · พลเมือง',
    'สื่อการสอนสังคมศึกษา ป.4-5: แผนที่ภาค สมัยสุโขทัย พลเมืองดี · รวมชิ้นกระจาย',
    v_url, '/games/social/social-thailand-hub/cover.png', 'สังคมศึกษา', ARRAY['ป.4','ป.5'], ARRAY['แผนที่','ประวัติศาสตร์','พลเมือง'], 100, false, true
  WHERE NOT EXISTS (SELECT 1 FROM public.educational_hub_items WHERE owner_staff_id = v_staff_id AND external_url = v_url);
  UPDATE public.educational_hub_items SET title = '🇹🇭 คลังสังคมศึกษาไทย — แผนที่ · สุโขทัย · พลเมือง',
    thumbnail_url = '/games/social/social-thailand-hub/cover.png', sort_order = 100,
  ...
  VALUES (v_item_id, v_staff_id, 'คลังสังคม ป.4-5', ARRAY['แผนที่ภาค','สุโขทัย','พลเมืองดี'], 'v1.0.0', 'TEACHING-MEDIA-IDEAS #16')
