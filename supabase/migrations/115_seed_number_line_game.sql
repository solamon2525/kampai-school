-- 115_seed_number_line_game.sql
-- เกม "เส้นจำนวนแม่นเป้า" (number-line) — วิชาคณิตศาสตร์ ป.4–6 (กลไกใหม่: ลากหมุดบนเส้นจำนวน)
-- public/games/math/number-line.html (KAMPAI SDK + kampai-match online)
-- Idempotent (NOT EXISTS guard) + sync flags/thumbnail/bgm
DO $$
DECLARE
  v_staff_id  UUID;
  v_cat_games UUID;
  v_url       TEXT := '/games/math/number-line.html';
BEGIN
  ...
  SELECT v_staff_id, v_cat_games, 'link', 'เส้นจำนวนแม่นเป้า', v_url, 'คณิตศาสตร์', 12
  ...
  SET game_slug = 'number-line', tracked_game = true, is_published = true,
      thumbnail_url = '/games/math/number-line-cover.svg', bgm_preset = 'cheerful', updated_at = now()
