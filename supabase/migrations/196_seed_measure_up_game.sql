-- ============================================================================
-- Migration 196: Seed "วัดและเปรียบเทียบ (Measure Up!)" (measure-up)
-- ============================================================================
-- Owner: ครูณัฐพงศ์ สิงห์ชมภู
-- Path: public/games/math/measure-up/index.html
-- Cover: public/games/math/measure-up/cover.svg
...
DECLARE
  v_staff_id  UUID;
  v_cat_games UUID;
  v_url       TEXT := '/games/math/measure-up/index.html';
BEGIN
  -- 1. Resolve staff_id
  ...
  SELECT v_staff_id, v_cat_games, 'link', '📏 วัดและเปรียบเทียบ (Measure Up!)', v_url, 'คณิตศาสตร์', 150
  WHERE NOT EXISTS (
  ...
  SET game_slug = 'measure-up',
      tracked_game = true,
      is_published = true,
      thumbnail_url = '/games/math/measure-up/cover.svg',
      updated_at = now()
  WHERE owner_staff_id = v_staff_id AND external_url = v_url;
END $$;
