...
- **เกม AR/กล้อง (เคลื่อนไหวร่างกาย):** อ่าน **`AR-GAME.md` ก่อนทำทุกครั้ง** + ใช้ engine กลาง `public/games/kampai-ar.js` (`KampaiAR`) + `cp -r public/games/_template-ar` (ตัวอย่าง `demo/ar-zone-quiz/`). 🔴 pitfall #1: container กล้องต้อง `position:absolute; inset:0` ห้าม `relative` (ยุบ 0px จอดำ/แตะไม่ได้ — เคส fraction-garden-ar) · บังคับมี tap fallback (เครื่องไม่มีกล้องต้องเล่นได้) · `verify:game` Check 10 = AR
