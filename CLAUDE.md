...
- **เกม AR/กล้อง (เคลื่อนไหวร่างกาย):** อ่าน **`AR-GAME.md` ก่อนทำทุกครั้ง** · **จิ้ม/ชนวัตถุด้วยนิ้ว** → `public/games/kampai-hands.js` (`KampaiHands`) + `cp -r public/games/_template-ar-hands` (ตัวอย่าง `thai/balloon-burst/`) · **ยืนเลือกโซน/ท่าทาง** → `kampai-ar.js` (`KampaiAR`) + `_template-ar` (ตัวอย่าง `demo/ar-zone-quiz/`) · 🔴 pitfall #1: container กล้องต้อง `position:absolute; inset:0` ห้าม `relative` · บังคับมี tap fallback · `verify:game` Check 10 = AR
