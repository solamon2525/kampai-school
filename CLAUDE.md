...
- **เกมคะแนนไม่ขึ้นใน portal (incident pizza-master-chef):** อ่าน **GAME.md §กฎเก็บคะแนน** ก่อน debug — สรุป: (1) `KAMPAI.setSlug` ตรง DB (2) รอบใหม่ต้อง `_submitted=false` + `postMessage gameStart` + ซ่อน game-over (3) `submitScore` เฉพาะตอนจบรอบจริง (4) โหมด practice ห้ามส่ง + ซ่อนใน embed (5) เล่นผ่าน `/play/slug` ไม่ใช่ HTML ตรง (6) สมุดเกรดเก็บ/กลาง/ปลาย = ครูโอนเอง ไม่ใช่ XP อัตโนมัติ
