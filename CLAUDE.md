...
- **เกมใหม่ใช้ KAMPAI SDK:** `/games/kampai-sdk.js` (single source) = `window.KAMPAI` — `setSlug/onReady/student/stats/leaderboard/submitScore/goHome/controls.mount(D-pad)`. wrapper (`PlayGame.tsx`) ส่ง student+stats+leaderboard ผ่าน init ให้เกมโชว์ในจอ. สั่ง AI เจ้าอื่นสร้างเกม → ใช้ `public/GAME-PROMPT.md` (ปุ่มดาวน์โหลด+คัดลอกใน GamesTab). เทมเพลต `_template-full.html`/`_template-react.html` ใช้ SDK แล้ว
