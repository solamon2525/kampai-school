...
5. **สเปคทางเทคนิคและสัดส่วนภาพ (Technical Specification):**
   ...
   - **บทเรียนและแนวทางการป้องกันตัวหนังสือหลุดขอบ (Text Alignment & Vertical Safe Zone Lesson)**: ตัวหนังสือชื่อเกมจะต้องไม่อยู่ชิดขอบบนหรือขอบล่างของภาพต้นฉบับ 1:1 เกินไป เพราะสคริปต์ `make-cover.mjs` จะทำการตัดพื้นที่ด้านบนและด้านล่างออกอย่างน้อยด้านละ 27% เสมอ ดังนั้นเพื่อป้องกันตัวหนังสือหลุดขอบ ให้ใส่เงื่อนไขกำกับใน prompt เพิ่มเติมว่า *"The text must be placed strictly in the center or lower-middle of the image. The top 30% and bottom 30% of the image must be plain background with absolutely no text or character heads."* เพื่อล็อกตำแหน่งให้ตัวอักษรอยู่ในช่วงกลาง 40% ของภาพเท่านั้น
...
## 4. AR Educational Game Development & SDK Integration Guidelines (แนวทางการพัฒนาเกม AR)

เมื่อพัฒนา Refactor หรือทดสอบเกมที่ใช้กล้อง (AR) หรือการตรวจจับการเคลื่อนไหวของร่างกาย ต้องปฏิบัติตามกฎเพื่อลดความเสี่ยงในการเกิด Runtime Crash และเสริมความมั่นคงของเกมดังนี้:

### A. JSDOM & Headless Environment Testing Compatibility
- **Canvas Context Safe-Guard**: สำหรับขั้นตอนการทดสอบ ( verify:game หรือ integration tests) ที่รันบน JSDOM หรือสภาพแวดล้อม Headless ที่ไม่มี GPU จริง:
  - ต้องตรวจสอบและ mock / stub ฟังก์ชันการทำงานของ Canvas และการตรวจจับภาพ (`HTMLCanvasElement.prototype.getContext`) ให้ส่งคืน Dummy Object Proxy ที่มีคำสั่งรองรับกราฟิกครบถ้วน เพื่อป้องกันไม่ให้เกมแครชตอนประเมินผลหน้าแรก
- **Camera Access on User Gesture Only**: ห้ามทำการเรียกขอใช้กล้องเว็บแคมผ่าน `ar.start()` เมื่อตอนโหลดสคริปต์ทันที (Page Load) แต่จะต้องขอกล้องเฉพาะเมื่อนักเรียนกดปุ่มเริ่มต้นเล่นเกมอย่างชัดเจน (User Gesture) เพื่อให้สามารถรัน JSDOM Smoke-test ผ่าน และป้องกันนโยบายความปลอดภัยกล้องของเว็บบราวเซอร์สมัยใหม่

### B. Mobile Browser Touch Fallback & UX Responsive
- **Touchstart Event Binding**: สำหรับพิกัดตระกร้าหรือวัตถุใดๆ ที่ขยับตามนิ้วหรือตำแหน่งยืนของผู้เล่น:
  - นอกจากการดักจับ `touchmove` หรือ `mousemove` แล้ว **ต้องทำการผูก event `touchstart` ควบคู่ด้วยเสมอ** เพื่อให้จุดชี้หรือตะกร้าตอบสนองต่อพิกัดแตะนิ้วครั้งแรกทันทีโดยไม่ต้องลากนิ้วก่อน ช่วยสร้างการควบคุมที่ลื่นไหลบนแท็บเล็ตและอุปกรณ์หน้าจอสัมผัสของโรงเรียน
- **Manual Tap Fallback**: เครื่องแท็บเล็ตและคอมพิวเตอร์พกพาส่วนมากในสถานศึกษาอาจไม่มีกล้องหรือปิดการอนุญาตใช้กล้อง จึงบังคับให้ทุกเกม AR จะต้องออกแบบมาให้สามารถแตะ/คลิกเพื่อเล่นได้ปกติเสมือนโหมด Solo เสมอ โดยการเรียกผ่าน `ar.tap(zone)`

### C. Resource Cleanup & Memory Leak Guardrails
- **Clear All Intervals and Transition Timeouts**: เมื่อมีการหยุดเกมชั่วคราว (Pause), สลับรอบการเล่น (Round Transition) หรือออกเกม (Exit/Quit):
  - ต้องทำการสั่งเคลียร์และล้าง Timeout ของฟีดแบ็กหรือช่วงพักรอสปอนข้อถัดไป (เช่น `ST.nextRoundTimeout` หรือ `state.feedbackTimeoutId`) และสปอนจับเวลาตัวเลข (`ST.spawnTimer`) ทั้งหมดอย่างหมดจด ป้องกันการสั่งงานสุ่มโหลดรอบถัดไปขึ้นมาในเบื้องหลัง
- **Proportional Coordinates Comparison Rule**: ในการคำนวณตำแหน่งวัตถุตกนอกจอ ห้ามเปรียบเทียบพิกัดพิกเซลจริง (เช่น `iy = it.y * H` พิกเซล) กับค่าคงที่อัตราส่วนทศนิยมตรงๆ (เช่น `iy > 1.05`) เพราะจะทำให้วัตถุถูกมองว่าตกนอกจอทันทีตั้งแต่สปอน ให้เปรียบเทียบด้วยอัตราส่วน `it.y > 1.05` หรือเปรียบเทียบพิกเซลตรงๆ `iy > 1.05 * H` เท่านั้น

### D. Versus Mode Deterministic RNG
- ในการจับคู่ประลองออนไลน์ซิงค์สด (Versus) ต้องทำการสุ่มโจทย์และสปอนวัตถุทั้งหมดผ่าน Seeded PRNG (`Mulberry32(seed)`) ที่ส่งมาจาก SDK กลาง เพื่อรับประกันความยุติธรรมสูงสุดในการแข่งขันคณิตศาสตร์และวิชาอื่นๆ โดยผู้ประลองทั้งสองฝ่ายต้องเผชิญกับโจทย์และอุปสรรคเดียวกันแบบ 100%
